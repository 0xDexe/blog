...
// Storage abstraction layer with fallback
const storage = {
...
async function loadBlogPosts() {
    ...
    for (const post of blogPosts) {
        try {
            ...
            // Get like count (will use initial value if not set)
...
async function openBlogPost(slug) {
    ...
    try {
        ...
        // Load like count and status with initial value from frontmatter
        const initialLikes = parseInt(frontmatter.likes || 0);
...
function closeBlogPost() {
    ...
    // Update the blog cards with new like counts
    loadBlogPosts();
}
...
async function getLikeCount(slug, initialCount = 0) {
    ...
    try {
        ...
        // If no stored value, initialize with the base value
        if (initialCount > 0) {
            await storage.set(key, initialCount.toString(), true);
        }
        return initialCount;
    } catch (error) {
        return initialCount;
...
async function toggleLike() {
    ...
    const isLiked = await getIsLiked(slug);
    const currentCount = await getLikeCount(slug, 0);
    
    const newIsLiked = !isLiked;
    const newCount = isLiked ? Math.max(0, currentCount - 1) : currentCount + 1;
    
    try {
        // Update like count (shared)
        await storage.set(`blog-likes-${slug}`, newCount.toString(), true);
        
        // Update user's like status (personal)
        await storage.set(`blog-liked-${slug}`, newIsLiked.toString(), false);
...
async function toggleBlogLike(slug, button, initialCount = 0) {
    const isLiked = await getIsLiked(slug);
    const currentCount = await getLikeCount(slug, initialCount);
